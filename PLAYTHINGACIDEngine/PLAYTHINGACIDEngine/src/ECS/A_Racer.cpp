...
static inline sf::Vector2f clampMag(sf::Vector2f v,
                                    float max) {
 float L = length(v); 
 if (L > max && L > 0.f) v *= (max / L); 
 return v;
}
...
void
A_Racer::setWaypoints(const std::vector<sf::Vector2f>& wps) {
 m_waypoints = wps;
